...
int main( int argc, char *argv[])
{
		...
		free_udp_port = mmap(NULL, sizeof *free_udp_port, PROT_READ | PROT_WRITE,
		...
		while(listen(tcp_fd, MAX_PENDING) != -1)
		{
				...
				if(pid == 0)
				{
						...
						if (type == 1 && !strcmp(message, "UDP PORT TO BE GIVEN"))
						{
								...
								encode_message(4, msg_for_cli, reply_message);

								sendto(udp_fd, reply_message, strlen(reply_message), 0, (struct sockaddr *) &client_address, sizeof(client_address));
								printf("Message sent: %s\n", reply_message);
